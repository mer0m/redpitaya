...
void plat_pid_vco_bypass_pid_only_release(struct device *dev)
{
...
static struct resource switchComplex_pid2_input_resources[] = {
	{
		.start = 0x43C10000,
...
static struct resource switchComplex_vco_resources[] = {
	{
		.start = 0x43D80000,
...
static struct plat_swC_port plat_switchComplex_pid2_input_data = {
	.name		= "switchComplex_pid2_input",
	.num		= 0,
	.idnum		= 0,
};
static struct plat_swC_port plat_switchComplex_vco_data = {
	.name		= "switchComplex_vco",
	.num		= 1,
	.idnum		= 1,
};
...
static struct platform_device plat_adc1_offset_device = {
	...
	.id	= 0,
	.dev	= {
		.release	= plat_pid_vco_bypass_pid_only_release,
...
static struct platform_device plat_switchComplex_pid2_input_device = {
	.name = "switchComplex",
	.id	= 0,
	.dev	= {
		.release	= plat_pid_vco_bypass_pid_only_release,
		.platform_data	= &plat_switchComplex_pid2_input_data,
	},
	.num_resources		= ARRAY_SIZE(switchComplex_pid2_input_resources),
	.resource		= switchComplex_pid2_input_resources,
};
static struct platform_device plat_switchComplex_vco_device = {
	.name = "switchComplex",
	.id	= 1,
	.dev	= {
		.release	= plat_pid_vco_bypass_pid_only_release,
		.platform_data	= &plat_switchComplex_vco_data,
	},
	.num_resources		= ARRAY_SIZE(switchComplex_vco_resources),
	.resource		= switchComplex_vco_resources,
};

static int __init board_pid_vco_bypass_pid_only_init(void)
{
	int ret;
	...
	if (ret < 0)
		return ret;
	ret = platform_device_register(&plat_switchComplex_pid2_input_device);
	if (ret < 0)
		return ret;
	ret = platform_device_register(&plat_switchComplex_vco_device);
	if (ret < 0)
		return ret;

	return ret;
}
static void __exit board_pid_vco_bypass_pid_only_exit(void)
{
	...
	platform_device_unregister(&plat_switchComplex_pid2_input_device);
	platform_device_unregister(&plat_switchComplex_vco_device);
}

module_init(board_pid_vco_bypass_pid_only_init);
module_exit(board_pid_vco_bypass_pid_only_exit);
MODULE_AUTHOR("toto <toto@toto.com>");
MODULE_DESCRIPTION("Board specific pid_vco_bypass_pid_only");
MODULE_LICENSE("GPL");
