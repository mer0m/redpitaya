...
void plat_pid_vco_amp_mod_pid_only_release(struct device *dev)
{
...
static struct platform_device plat_adc1_offset_device = {
	...
	.id	= 0,
	.dev	= {
		.release	= plat_pid_vco_amp_mod_pid_only_release,
...
static int __init board_pid_vco_amp_mod_pid_only_init(void)
{
	int ret;
...
static void __exit board_pid_vco_amp_mod_pid_only_exit(void)
{
...
module_init(board_pid_vco_amp_mod_pid_only_init);
module_exit(board_pid_vco_amp_mod_pid_only_exit);
MODULE_AUTHOR("toto <toto@toto.com>");
MODULE_DESCRIPTION("Board specific pid_vco_amp_mod_pid_only");
MODULE_LICENSE("GPL");
