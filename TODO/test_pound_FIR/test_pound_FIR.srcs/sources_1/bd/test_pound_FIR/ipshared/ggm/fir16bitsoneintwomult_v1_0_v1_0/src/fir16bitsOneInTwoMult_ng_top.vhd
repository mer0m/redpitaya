...
---------------------------------------------------------------------------
	...
	-- data output
	signal data_out_i_s, data_out_i_next : std_logic_vector(DATA_OUT_SIZE-1 downto 0);
	signal data_out_q_s, data_out_q_next : std_logic_vector(DATA_OUT_SIZE-1 downto 0);
	...
	signal result_i_s : result_tab(NB_FIR-1 downto 0);
	signal result_q_s : result_tab(NB_FIR-1 downto 0);
...
begin
	...
	begin
		if rising_edge(processing_clk_i) then
			...
			if data_en_next = '1' then
				data_out_q_s <= data_out_q_next;
				data_out_i_s <= data_out_i_next;
				data_en_o <=  '1';
			end if;
		end if;
	end process;
	...
	process(data_out_i_s, result_i_s, result_en_s)
	...
	process(data_out_q_s, result_q_s, result_en_s)
		variable tmp : std_logic_vector(DATA_OUT_SIZE-1 downto 0);
	begin
		tmp := data_out_q_s;
		boucle_test :for i in 0 to NB_FIR-1 loop
	...
	generate_fir : for i in 0 to NB_FIR-1 generate
		...
		(
			...
			-- for the output
			data_i_o	=> result_i_s(i),
			data_q_o	=> result_q_s(i),
	...
	port map (clk_a => clk, clk_b => processing_clk_i,
		reset_a => reset, reset_b => processing_rst_i,
		-- input
