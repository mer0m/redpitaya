...
Entity red_pitaya_pidv2_logic is 
	generic (
		P_SIZE : integer := 16;
	...
	(
		...
		kp_i	   : in std_logic_vector(D_SIZE-1 downto 0);
...
Architecture behavioral of red_pitaya_pidv2_logic is
	--sig
	...
	signal I_sum_s	  	: std_logic_vector(I_MULT_SZ-1 downto 0); --somme
	signal I_temp_s   	: std_logic_vector(I_MULT_SZ-1 downto 0); --
	...
	signal pid_sum_s  : std_logic_vector(PID_SZ-1 downto 0);	--v_k
...
----
	...
	begin
		if rising_edge(clk_i) then
			if (reset or int_rst_i) = '1' then --ici aussi le reset_int
				...
				I_temp_s <= I_temp_s;
				if data_en_s = '1' then
					--positive saturation --on devrait foutre ça au niveau de la somme
					if (I_sum_s(I_MULT_SZ-1 downto I_MULT_SZ-2) = "01") then
											--I_s <= '0' & (31 downto 0 => '1');
						I_s <= '0' & (I_RES_SZ-2 downto 0 => '1');
					elsif (I_sum_s(I_MULT_SZ-1 downto I_MULT_SZ-2) = "10") then --negative saturation
											--I_s <= '1' & (31 downto 0 => '0');
						I_s <= '1' & (I_RES_SZ-2 downto 0 => '0');  --33-1 dt ISR
											--else I_s <= I_sum_s; --desaturation ici ?
					else 
						I_s <= I_sum_s(I_MULT_SZ-1 downto ISR);
						I_temp_s <= I_sum_s;
					end if;
				end if;
			end if;
	...
	I_sum_s <= std_logic_vector(signed(I_temp_s) + (signed(ki_i)*signed(error_s)) ); --
		...
		if rising_edge(clk_i) then
			...
			if reset = '1' then
				...
				if data2_en_s ='1' then
					data_en_o <= '1';
					if (pid_sum_s(PID_SZ-1 downto PID_SZ -2) = "01") then
					...
					elsif ( pid_sum_s (PID_SZ-1 downto PID_SZ-2) = "10") then
						--pid_out_s <= '1' & (others => '0');
						--pid_out_s <= '1' & (DATA_IN_SIZE-2 downto 0 => '0');  --borne -1V
						pid_out_s <=  '1' & (PID_SZ-2 downto 0 => '0');  --borne 0V
					else
						pid_out_s <= pid_sum_s;
					end if;
				end if;
	...
	pid_sum_s <= std_logic_vector( (PID_SZ-1 downto 0 => '0') + signed(p_pid_s) + signed(i_pid_s));
