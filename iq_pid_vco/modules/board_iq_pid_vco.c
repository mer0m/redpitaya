...
void plat_iq_pid_vco_release(struct device *dev)
{
...
static struct platform_device plat_adc1_offset_device = {
	...
	.id	= 0,
	.dev	= {
		.release	= plat_iq_pid_vco_release,
...
static int __init board_iq_pid_vco_init(void)
{
	int ret;
...
static void __exit board_iq_pid_vco_exit(void)
{
...
module_init(board_iq_pid_vco_init);
module_exit(board_iq_pid_vco_exit);
MODULE_AUTHOR("toto <toto@toto.com>");
MODULE_DESCRIPTION("Board specific iq_pid_vco");
MODULE_LICENSE("GPL");
