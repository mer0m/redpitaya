...
void plat_demod_pid_vco_amp_mod_pid_only_release(struct device *dev)
{
...
static struct resource adc2_offset_resources[] = {
	{
		.start = 0x43D80000,
		.end   = 0x43D80000 + 0xffff,
		.flags = IORESOURCE_MEM,
	},
};

static struct resource pid3_kp_resources[] = {
	{
		.start = 0x43D90000,
		.end   = 0x43D90000 + 0xffff,
		.flags = IORESOURCE_MEM,
	},
};

static struct resource pid3_ki_resources[] = {
	{
		.start = 0x43DA0000,
		.end   = 0x43DA0000 + 0xffff,
		.flags = IORESOURCE_MEM,
	},
};

static struct resource pid3_kd_resources[] = {
	{
		.start = 0x43DB0000,
		.end   = 0x43DB0000 + 0xffff,
		.flags = IORESOURCE_MEM,
	},
};

static struct resource pid3_setpoint_resources[] = {
	{
		.start = 0x43DC0000,
		.end   = 0x43DC0000 + 0xffff,
		.flags = IORESOURCE_MEM,
	},
};

static struct resource pid3_sign_resources[] = {
	{
		.start = 0x43DD0000,
		.end   = 0x43DD0000 + 0xffff,
		.flags = IORESOURCE_MEM,
	},
};

static struct resource pid3_rst_int_resources[] = {
	{
		.start = 0x43DE0000,
		.end   = 0x43DE0000 + 0xffff,
		.flags = IORESOURCE_MEM,
	},
};

static struct resource mod_amp_dds_ampl_resources[] = {
	{
		.start = 0x43DF0000,
		.end   = 0x43DF0000 + 0xffff,
		.flags = IORESOURCE_MEM,
	},
};
...
static struct resource mod_amp_dds_nco_resources[] = {
	{
		.start = 0x43E00000,
		.end   = 0x43E00000 + 0xffff,
		.flags = IORESOURCE_MEM,
	},
};
...
static struct plat_add_const_port plat_pid3_kp_data = {
	.name		= "pid3_kp",
	.num		= 21,
	.idnum		= 21,
};
static struct plat_add_const_port plat_pid3_ki_data = {
	.name		= "pid3_ki",
	.num		= 22,
	.idnum		= 22,
};
static struct plat_add_const_port plat_pid3_kd_data = {
	.name		= "pid3_kd",
	.num		= 23,
	.idnum		= 23,
};
static struct plat_add_const_port plat_pid3_setpoint_data = {
	.name		= "pid3_setpoint",
	.num		= 24,
	.idnum		= 24,
};
static struct plat_add_const_port plat_pid3_sign_data = {
	.name		= "pid3_sign",
	.num		= 25,
	.idnum		= 25,
};
static struct plat_add_const_port plat_pid3_rst_int_data = {
	.name		= "pid3_rst_int",
	.num		= 26,
	.idnum		= 26,
};
static struct plat_add_const_port plat_mod_amp_dds_ampl_data = {
	.name		= "mod_amp_dds_ampl",
	.num		= 27,
	.idnum		= 27,
};
...
static struct plat_nco_counter_port plat_mod_amp_dds_nco_data = {
	.name		= "mod_amp_dds_nco",
	.num		= 3,
	.idnum		= 3,
};
...
static struct platform_device plat_adc1_offset_device = {
	...
	.id	= 0,
	.dev	= {
		.release	= plat_demod_pid_vco_amp_mod_pid_only_release,
...
static struct platform_device plat_pid3_kp_device = {
	.name = "add_const",
	.id	= 21,
	.dev	= {
		.release	= plat_demod_pid_vco_amp_mod_pid_only_release,
		.platform_data	= &plat_pid3_kp_data,
	},
	.num_resources		= ARRAY_SIZE(pid3_kp_resources),
	.resource		= pid3_kp_resources,
};
static struct platform_device plat_pid3_ki_device = {
	.name = "add_const",
	.id	= 22,
	.dev	= {
		.release	= plat_demod_pid_vco_amp_mod_pid_only_release,
		.platform_data	= &plat_pid3_ki_data,
	},
	.num_resources		= ARRAY_SIZE(pid3_ki_resources),
	.resource		= pid3_ki_resources,
};
static struct platform_device plat_pid3_kd_device = {
	.name = "add_const",
	.id	= 23,
	.dev	= {
		.release	= plat_demod_pid_vco_amp_mod_pid_only_release,
		.platform_data	= &plat_pid3_kd_data,
	},
	.num_resources		= ARRAY_SIZE(pid3_kd_resources),
	.resource		= pid3_kd_resources,
};
static struct platform_device plat_pid3_setpoint_device = {
	.name = "add_const",
	.id	= 24,
	.dev	= {
		.release	= plat_demod_pid_vco_amp_mod_pid_only_release,
		.platform_data	= &plat_pid3_setpoint_data,
	},
	.num_resources		= ARRAY_SIZE(pid3_setpoint_resources),
	.resource		= pid3_setpoint_resources,
};
static struct platform_device plat_pid3_sign_device = {
	.name = "add_const",
	.id	= 25,
	.dev	= {
		.release	= plat_demod_pid_vco_amp_mod_pid_only_release,
		.platform_data	= &plat_pid3_sign_data,
	},
	.num_resources		= ARRAY_SIZE(pid3_sign_resources),
	.resource		= pid3_sign_resources,
};
static struct platform_device plat_pid3_rst_int_device = {
	.name = "add_const",
	.id	= 26,
	.dev	= {
		.release	= plat_demod_pid_vco_amp_mod_pid_only_release,
		.platform_data	= &plat_pid3_rst_int_data,
	},
	.num_resources		= ARRAY_SIZE(pid3_rst_int_resources),
	.resource		= pid3_rst_int_resources,
};
static struct platform_device plat_mod_amp_dds_ampl_device = {
	.name = "add_const",
	.id	= 27,
	.dev	= {
		.release	= plat_demod_pid_vco_amp_mod_pid_only_release,
		.platform_data	= &plat_mod_amp_dds_ampl_data,
	},
	.num_resources		= ARRAY_SIZE(mod_amp_dds_ampl_resources),
	.resource		= mod_amp_dds_ampl_resources,
};
...
static struct platform_device plat_mod_amp_dds_nco_device = {
	.name = "nco_counter",
	.id	= 3,
	.dev	= {
		.release	= plat_demod_pid_vco_amp_mod_pid_only_release,
		.platform_data	= &plat_mod_amp_dds_nco_data,
	},
	.num_resources		= ARRAY_SIZE(mod_amp_dds_nco_resources),
	.resource		= mod_amp_dds_nco_resources,
};
...
static int __init board_demod_pid_vco_amp_mod_pid_only_init(void)
{
	int ret;
	...
	if (ret < 0)
		return ret;
	ret = platform_device_register(&plat_pid3_kp_device);
	if (ret < 0)
		return ret;
	ret = platform_device_register(&plat_pid3_ki_device);
	if (ret < 0)
		return ret;
	ret = platform_device_register(&plat_pid3_kd_device);
	if (ret < 0)
		return ret;
	ret = platform_device_register(&plat_pid3_setpoint_device);
	if (ret < 0)
		return ret;
	ret = platform_device_register(&plat_pid3_sign_device);
	if (ret < 0)
		return ret;
	ret = platform_device_register(&plat_pid3_rst_int_device);
	if (ret < 0)
		return ret;
	ret = platform_device_register(&plat_mod_amp_dds_ampl_device);
	if (ret < 0)
		return ret;
	...
	if (ret < 0)
		return ret;
	ret = platform_device_register(&plat_mod_amp_dds_nco_device);
	if (ret < 0)
		return ret;
...
static void __exit board_demod_pid_vco_amp_mod_pid_only_exit(void)
{
	...
	platform_device_unregister(&plat_pid3_kp_device);
	platform_device_unregister(&plat_pid3_ki_device);
	platform_device_unregister(&plat_pid3_kd_device);
	platform_device_unregister(&plat_pid3_setpoint_device);
	platform_device_unregister(&plat_pid3_sign_device);
	platform_device_unregister(&plat_pid3_rst_int_device);
	platform_device_unregister(&plat_mod_amp_dds_ampl_device);
	...
	platform_device_unregister(&plat_mod_amp_dds_nco_device);
...
module_init(board_demod_pid_vco_amp_mod_pid_only_init);
module_exit(board_demod_pid_vco_amp_mod_pid_only_exit);
MODULE_AUTHOR("toto <toto@toto.com>");
MODULE_DESCRIPTION("Board specific demod_pid_vco_amp_mod_pid_only");
MODULE_LICENSE("GPL");
