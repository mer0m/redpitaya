...
architecture Behavioral of data16_multi_to_ram_top is
	...
	-- start resync if needed --
	signal sync_start_s, sync_start1_s, sync_start2_s, sync_start3_s : std_logic;
begin

	sync_start_en : if SYNC_START = true generate
		process(processing1_clk_i)
		begin
			if rising_edge(processing1_clk_i) then
				if processing_rst_i(0) = '1' then
					sync_start1_s <= '0';
					sync_start2_s <= '0';
					sync_start3_s <= '0';
					sync_start_s <= '0';
				else
					sync_start1_s <= start_acquisition_i;
					sync_start2_s <= sync_start1_s;
					sync_start3_s <= sync_start2_s;
					sync_start_s <= sync_start3_s;
				end if;
			end if;
		end process;
	end generate sync_start_en;
	sync_start_dis : if SYNC_START = false generate
		sync_start1_s <= start_acquisition_i;
		sync_start2_s <= sync_start1_s;
		sync_start3_s <= sync_start2_s;
		sync_start_s <= sync_start3_s;
	end generate sync_start_dis;
	...
	subtop_loop : for i in 0 to NB_WAY-1 generate
		...
		port map (processing_clk_i => processing_clk_s(i), cpu_clk_i => cpu_clk_i,
			...
			start_acquisition_i => sync_start_s, busy_o => busy_s(i),
