...
void plat_double_pid_vco_release(struct device *dev)
{
...
static struct resource dds2_f0_resources[] = {
	{
		.start = 0x43D40000,
...
static struct resource dds2_range_resources[] = {
	{
		.start = 0x43D30000,
...
static struct plat_add_const_port plat_dds2_f0_data = {
	.name		= "dds2_f0",
	.num		= 7,
	.idnum		= 7,
};
...
static struct plat_add_const_port plat_dds2_range_data = {
	.name		= "dds2_range",
	.num		= 9,
	.idnum		= 9,
};
...
static struct platform_device plat_adc1_offset_device = {
	...
	.id	= 0,
	.dev	= {
		.release	= plat_double_pid_vco_release,
...
static struct platform_device plat_dds2_f0_device = {
	.name = "add_const",
	.id	= 7,
	.dev	= {
		.release	= plat_double_pid_vco_release,
		.platform_data	= &plat_dds2_f0_data,
	},
	.num_resources		= ARRAY_SIZE(dds2_f0_resources),
	.resource		= dds2_f0_resources,
};
...
static struct platform_device plat_dds2_range_device = {
	.name = "add_const",
	.id	= 9,
	.dev	= {
		.release	= plat_double_pid_vco_release,
		.platform_data	= &plat_dds2_range_data,
	},
	.num_resources		= ARRAY_SIZE(dds2_range_resources),
	.resource		= dds2_range_resources,
};
...
static int __init board_double_pid_vco_init(void)
{
	int ret;
	...
	if (ret < 0)
		return ret;
	ret = platform_device_register(&plat_dds2_f0_device);
	if (ret < 0)
		return ret;
	ret = platform_device_register(&plat_dds2_offset_device);
	if (ret < 0)
		return ret;
	ret = platform_device_register(&plat_dds2_range_device);
	if (ret < 0)
		return ret;
...
static void __exit board_double_pid_vco_exit(void)
{
	...
	platform_device_unregister(&plat_dds2_f0_device);
	platform_device_unregister(&plat_dds2_offset_device);
	platform_device_unregister(&plat_dds2_range_device);
...
module_init(board_double_pid_vco_init);
module_exit(board_double_pid_vco_exit);
MODULE_AUTHOR("toto <toto@toto.com>");
MODULE_DESCRIPTION("Board specific double_pid_vco");
MODULE_LICENSE("GPL");
